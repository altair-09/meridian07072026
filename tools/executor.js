...
import { trackSimPosition, listSimPositions } from "../sim-state.js";
import fs from "fs";
...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      simVirtualWalletSol: ["simulation", "virtualWalletSol", ["simVirtualWalletSol"]],
      // chart indicators
...
export async function executeTool(name, args) {
  ...
  try {
    ...
    if (name === "deploy_position" && result?.dry_run === true) {
      const openSimCount = listSimPositions({ openOnly: true }).length;
      if (openSimCount >= (config.simulation?.maxOpenPositions ?? 20)) {
        log("sim_state", `Skipped mirroring dry-run deploy into sim-state: at cap (${openSimCount} open)`);
      } else {
        try {
          trackSimPosition({
